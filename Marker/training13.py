...
if True: #sys.argv[1] == 'ext_feat':
	...
	y, file_paths = load_path("./data/test_rotation4/", 1, y, file_paths)
	y = y[:num+num]
...
model2.add(keras.layers.advanced_activations.PReLU(init='zero', weights=None))#act)
...
model2.add(layers.BatchNormalization())
#model2.add(layers.Activation('relu'))
model2.add(keras.layers.advanced_activations.PReLU(init='zero', weights=None))
model2.add(layers.MaxPooling2D(pool_size=pool1))
model2.add(layers.Dropout(0.25))
